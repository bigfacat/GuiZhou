 ...
 function tdzzs6_jmseQtlxfc(ssjmxzDm){
	  ...
	  for(var i = 0;i < sbxxGridlb.length;i++){
		  for(var j = 0;j < tdzzs_zspmdmzszmdm.length;j++){
			  if(sbxxGridlb[i].zszmMc==zszmMc&&zszmMc!=""&&zszmMc!=null){
				  if(tdzzs_zspmdmzszmdm[j].zszmMc == sbxxGridlb[i].zszmMc){
					  count++;
					  if(count == 2){
						  return false;
					  }
  ...
  function calculateNsqxDm(skssqq,skssqz){	  
	...
	/**
	   ...
	   * @returns
	   */
	function tdzzs4_sfQs(bdcxmbh, gridLb){
		...
		for(var i=0;i<xmbhs.length;i++){
			if(bdcxmbh == xmbhs[i].xmbh){
				return xmbhs[i].qsbz == "Y"?true:false;
			}
		}
		return true;
	}
...
function fjsgzsb_getIndex(jmxzDm,option,rdpzuuid) {
	...
	if(option!=undefined && option!=null){
		for(var i=0;i<option.length;i++){
			//var tempValue = i+"";
			if(option[i]!=null && option[i].dm==jmxzDm){
				jmxzIndex = i;
				return jmxzIndex+"";
			}
		}
