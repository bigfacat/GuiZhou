...
viewApp.filter('pzxh',
    function() {
        return function(item) {
            ...
            //凭证序号为 为null 导出盘报类
                return "<button class=\"layui-btn layui-btn-sm\" onclick=\"queryAndexport('"+item.id+"','3','"+item.gdslxDm+"')\">查看PDF</button>";
            else
            //正常申报类
            if(item.showType == "0"){
                if(item.sbzfbz == "Y"){
                    return "<button class=\"layui-btn layui-btn-sm\" onclick=\"warmInfo('0');\" >查看PDF</button>";
                }else{
                    return "<button class=\"layui-btn layui-btn-sm\" onclick=\"queryAndexport('"+item.pzxh+"','1','"+item.gdslxDm+"')\" >查看PDF</button>";
                }
            }else if(item.showType == "1"){
                if(item.sbzfbz == "Y"){
                    return "<button class=\"layui-btn layui-btn-sm\" onclick=\"warmInfo('1');\" >下载PDF</button>";
                }else{
                    return "<button class=\"layui-btn layui-btn-sm\" onclick=\"cxDownloadPDF('/zlpz-cjpt-web/zlpz/viewOrDownloadPdfFile.do?ysqxxid="+item.id+"&viewOrDownload=download&gdslxDm="+item.gdslxDm+"&ywbm="+item.ywbm+"')\" >下载PDF</button>";
                }
            }else if(item.showType == "2"){
                if(item.sbzfbz == "Y"){
                    return "<button style=\"margin-right:2px;\" class=\"layui-btn layui-btn-sm\" onclick=\"warmInfo('0');\" >查看PDF</button>" +
                        "<button class=\"layui-btn layui-btn-sm\" onclick=\"warmInfo('1');\" >下载PDF</button>";
                }else{
                    return "<button style=\"margin-right:2px;\" class=\"layui-btn layui-btn-sm\" onclick=\"queryAndexport('"+item.pzxh+"','1','"+item.gdslxDm+"')\" >查看PDF</button>" +
                        "<button class=\"layui-btn layui-btn-sm\" onclick=\"cxDownloadPDF('/zlpz-cjpt-web/zlpz/viewOrDownloadPdfFile.do?ysqxxid="+item.id+"&viewOrDownload=download&gdslxDm="+item.gdslxDm+"&ywbm="+item.ywbm+"')\" >下载PDF</button>";
                }
            }else{
                if(item.sbzfbz == "Y"){
                    return "<button class=\"layui-btn layui-btn-sm\" onclick=\"warmInfo('1');\" >下载PDF</button>";
...
viewApp.filter('id',
    function() {
        return function(item) {
            ...
            // 其他按钮预留
                return "<button class=\"layui-btn layui-btn-sm\" onclick=\"queryAndexport('"+item.id+"','2','"+item.gdslxDm+"')\">查看PDF</button>";
            else
                return "<button class=\"layui-btn layui-btn-sm\" onclick=\"queryAndexport('"+item.id+"','2','"+item.gdslxDm+"')\" >查看PDF</button>" ;
        };
    });
...
viewApp.filter('cksbb',
    function() {
        return function(item) {
            //非本地渠道处理
        	if(item.version == 4) {
                return "<button class=\"layui-btn layui-btn-sm\"  onclick='alertMsgs(\"非本渠道申报，暂不提供打印!\", {icon : 5})'>查看申报表</button>";
            } else {
                return "<button class=\"layui-btn layui-btn-sm\" onclick=\"cksbb('" + item.gnId + "','" + item.bdDm + "','" + item.id + "','" + item.gdslxDm + "','" + item.sjgsrq + "','" + item.sjgsdq + "')\">查看申报表</button>";
            }};
    });
 ...
 * @param gdslxDm
 */
function queryAndexport(pzxh, version,gdslxDm) {
    // alert("查看PDF");
