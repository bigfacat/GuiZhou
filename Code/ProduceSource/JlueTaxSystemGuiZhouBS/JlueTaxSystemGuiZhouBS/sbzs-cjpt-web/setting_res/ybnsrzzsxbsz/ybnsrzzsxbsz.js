...
parent.validateBeforeSubmit = function() {
	...
	if(frmData.jcxxsz.BDA0610566 === 'Y' && frmData.xsmap.WSPZDKJQD === 'Y' && frmData.jcxxsz.WSPZDKJQD !== 'Y'){
		parent.Message.errorInfo({
			title : "错误",
			message : "已勾选“增值税纳税申报表附列资料（表四）（税额抵减情况表）”，请同步勾选“完税凭证抵扣清单”！"
		});
		return false;
	}
	...
	if(frmData.jcxxsz.BDA0610338 === 'Y' && frmData.xsmap.FWBDCHWXZCKCXMQD === 'Y' && frmData.jcxxsz.FWBDCHWXZCKCXMQD !== 'Y'){
		parent.Message.errorInfo({
			title : "错误",
			message : "已勾选“增值税纳税申报表附列资料（表三）（服务、不动产和无形资产扣除项目明细）”，请同步勾选“服务、不动产和无形资产扣除项目清单”！"
		});
		return false;
	}
...
parent.saveDataCallback = function(data){
	...
	var url = parent.location.protocol + "//" + parent.location.host +  result +"/biz/sbzs/ybnsrzzs.html" + parent.location.search;
