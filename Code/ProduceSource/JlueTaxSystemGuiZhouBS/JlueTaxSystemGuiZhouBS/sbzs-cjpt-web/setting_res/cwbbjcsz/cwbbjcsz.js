...
function saveEvent2() {
	...
	$.ajax({
		type: "POST",
		url: parent.cp + "/setting/saveData.do?djxh=" + parent.$("#djxh").val() + "&ywbm=" + parent.$("#ywbm").val() + "&sssqQ=" + sssqQ + "&sssqZ=" + sssqZ + "&gdslxDm=" + parent.$("#gdslxDm").val() + "&test=" + parent.$("#test").val() + "&nsrsbh=" + parent.$("#nsrsbh").val() + "&swjgDm=" + parent.$("#swjgDm").val(),
		dataType: "json",
		...
		success: function (data) {
			...
			if ("Y" == jsondata.returnFlag) {
				var msg = jsondata.warnInfo ? jsondata.warnInfo.msg : false;
				//				var msg=jsondata.warnInfo.msg;
				if (msg) {
					parent.Message.succeedInfo({
						title: "提示",
						message: msg,
				...
				parent.Message.errorInfo({
					title: "错误",
					message: "尊敬的纳税人：G000000表中纳税人信息与当前登录纳税人信息不一致，请点击【确定】刷新页面！",
					handler: function () {
...
parent.saveDataCallback = function (data) {
					...
					success: function (redata) {
						if (redata != "" && redata != null && redata != undefined) {
							...
							if (code != "" && code != null && code != undefined) {
								...
								var nsrsbh = $('#nsrsbh', window.parent.document).val();
								var djxh = $('#djxh', window.parent.document).val();
								var nsrmc = getQueryString("nsrmc");
								var zgswskfjDm = getQueryString("zgswskfjDm");
								if (zgswskfjDm == null || zgswskfjDm == undefined || zgswskfjDm == "" || zgswskfjDm == "undefined") {
									zgswskfjDm = $('#swjgDm', window.parent.document).val(); //取税务机关代码
								}
							...
							parent.Message.errorInfo({
								title: "错误",
								message: "未获取到授权码，无法跳转到Excel导入财报页面！"
							});
						}
					},
					error: function () {
	...
	//debugger;
	//	if ($("#ywbm").val().match("CWBB")){
	//		url = location.protocol +"//" + location.host+$("#context").val()+"/nssb/cwbbmain.do";
	//		var urlstr = window.location.search;
	//		url += urlstr;
	//		var urlArgs = urlstr.split("&");
	//		for (var i = 0; i < urlArgs.length; i++){
	//			if (urlArgs[i].match("fromsbb")){
	//				//首页设置页进入，在新窗口打开
	...
	//从申报页点击进入，就在当前窗口打开
	//		window.open(url);
	//		window.showModalDialog(url,"","dialogHeight:"+screen.height+";dialogWidth:"+screen.width+";");
	//	}
...
function getDzbd(zlbsxlDm) {
	...
	var url = parent.location.href;
	var djxhIndex = url.indexOf("djxh=");
	var djxh = "";
	if (djxhIndex != -1) {
		if (url.indexOf("&", djxhIndex) != -1) {
			djxh = url.substring(djxhIndex + "djxh=".length, url.indexOf("&", djxhIndex));
		} else {
			djxh = url.substring(djxhIndex + "djxh=".length);
		}
	}
	...
	var gdslxDm = "";
	var gdslxDmIndex = url.indexOf("gdslxDm=");
	if (gdslxDmIndex != -1) {
		if (url.indexOf("&", gdslxDmIndex) != -1) {
			gdslxDm = url.substring(gdslxDmIndex + "gdslxDm=".length, url.indexOf("&", gdslxDmIndex));
		} else {
			gdslxDm = url.substring(gdslxDmIndex + "gdslxDm=".length);
		}
	}
...
function checkIsQhjt(scope, jsonData) {
	...
	pramData.djxh = jsonData.djxh;
	pramData.gdslxDm = jsonData.gdslxDm;
	pramData.nsrsbh = parent.$("#nsrsbh").val();
...
function saveEvent1() {
	...
	cwqxData.djxh = parent.$("#djxh").val();
	cwqxData.gdslxDm = parent.$("#gdslxDm").val();
	cwqxData.swjgDm = parent.$("#swjgDm").val();
	$.ajax({
		type: "POST",
