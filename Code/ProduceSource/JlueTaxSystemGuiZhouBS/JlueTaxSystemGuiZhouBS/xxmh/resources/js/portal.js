...
function showNsrs(nsrQysqVos,qybdid,isZrrsfJr,yhm,zjhm,zjlx,zjyxq,zrrs){
	...
	for(var i in nsrQysqVos){
		...
		if(qybdid=="" && isZrrsfJr=="N"){
			...
			active=(nsrQysqVos[i].qybdid==qybdid?'active':'');
		}
		if(nsrQysqVos[i].dsnsrsbh!=null&&nsrQysqVos[i].dsnsrsbh!=""&&typeof(nsrQysqVos[i].dsnsrsbh)!="undefined"&&nsrQysqVos[i].gsnsrsbh!=null&&nsrQysqVos[i].gsnsrsbh!=""&&typeof(nsrQysqVos[i].gsnsrsbh)!="undefined"){
			nsrztHtml = '<li class="'+(i==0?'active':'')+'" qybdid='+nsrQysqVos[i].qybdid+'>'+nsrQysqVos[i].nsrmc+'（'+nsrQysqVos[i].zzNsrmc+'-'+nsrQysqVos[i].yhsfmc+'）  </li>';
		}else if(nsrQysqVos[i].gsnsrsbh!=null&&nsrQysqVos[i].gsnsrsbh!=""&&typeof(nsrQysqVos[i].gsnsrsbh)!="undefined"){
			nsrztHtml = '<li class="'+(i==0?'active':'')+'" qybdid='+nsrQysqVos[i].qybdid+'>'+nsrQysqVos[i].nsrmc+'（'+nsrQysqVos[i].nsrmc+'-'+nsrQysqVos[i].yhsfmc+'）  （国税）</li>';
		}else if(nsrQysqVos[i].dsnsrsbh!=null&&nsrQysqVos[i].dsnsrsbh!=""&&typeof(nsrQysqVos[i].dsnsrsbh)!="undefined"){
			nsrztHtml = '<li class="'+(i==0?'active':'')+'" qybdid='+nsrQysqVos[i].qybdid+'>'+nsrQysqVos[i].dsNsrmc+'（'+nsrQysqVos[i].dsNsrmc+'-'+nsrQysqVos[i].yhsfmc+'）  （地税）</li>';
		}
	...
	if(qykeywords!=""){
		...
		$("#nsrzt li").parent().children(".active").removeClass("active");
		$("#nsrzt li").parent().children(":visible").first().addClass("active");	
	}else{
		$("#nsrzt li").show()
	}
...
function selectQy(){
	...
	var qybdid=$("#nsrzt").find("li.active").attr("qybdid");
	if(!qybdid){
...
function grsfjr(){
    ...
    var zrrDjxh = '';
    if (length > 0) {
		zrrDjxh = $("#zrrsList").find("li.active").attr("zrrdjxh");
			...
			if(d.isZrrsfJr=="N"){
				if(d.zztList && d.zztList.length > 0){
					...
					if(chr) {
						...
						layer.alert("您尚未绑定任何主管税务机关", {title:"提示",icon: 5});
					}
				}
			}
		}
...
function showZztsfqhDialog(zztList,gsDjxh,dsDjxh){
	...
	for(var i in zztList){
        ...
        var html = '<tr>'
            ...
            +'<div class="layui-form-item ">'
            +'<input type="checkbox" id="radio'+i+'" name="radio" lay-skin="primary" value="'+djxh+'" checked="">'
...
//	$("#zztIdUL").empty();
    zztsfIndex=layer.open({
        ...
        success:function(){
            layui.form.render(null,'zztIdUL');//刷新表单中动态样式
            layui.form.on('checkbox(sfxzCheckbox)', function(data){
                if(data.elem.checked==true){
                    $("#zztIdUL input[type='checkbox']").not(data.elem).each(function(){
                        $(this).prop("checked",false);
                    });
                    layui.form.render("checkbox","zztIdUL");
                }
            });
        }
    });
...
function seachZgswjg(){
	...
	if(zgswjgkeywords!=""){
		$(".searchTrColor").show().each(function(){
			var text=$(this).children("td:eq(1)").text();
			if(text.search(zgswjgkeywords)==-1){
				$(this).hide();
			}
		})
		$(".searchTrColor :radio").attr("checked",false); 
	}else{
		$(".searchTrColor").show()
	}
}
...
function login(){
	...
	$("*[name='logTip2']").hide();
	$("*[name='logTip1']").hide();*/
	//window.location.href=contPath+"/page/login.html?t="+(+new Date());//登录页面需要判断是否已经登录，否则进入单点登录页面

	var ssoXxmhUrl=$("#ssoXxmhUrl").val();
	if(typeof(ssoXxmhUrl) =="undefined" || ssoXxmhUrl == "" ){
		window.location.href.split(".")[0].split("//")[1];
		var ssoXxmhUrl = "http://" + window.location.host + "/sso/login?service=http://" + window.location.host + "/xxmh/html/index_new.html?bszmFrom=1&t="+(+new Date());
	}
	ssoXxmhUrl = "http://" + window.location.host + "/sso/login?service=http://" + window.location.host + "/xxmh/html/index_new.html?bszmFrom=1&t="+(+new Date());
...
//			console.log(menuHtml2);
			...
			if(menuHtml!=""){
				...
				if(isNSR == "Y"){ //企业用户
					if(hasYhgl=="Y"){//
						$("#userName").attr("href","javascript:window.goPage('','yhgl','xgmm');");
					}else{
						$("#userName").attr("href","javascript:alertMessgae('用户未分配用户权限！')");
					}
				}else{ //个人用户
					if(hasYhgl=="Y"){//
						$("#userName").attr("href","javascript:window.goPage('','yhgl','zhxx');");
					}else{
						$("#userName").attr("href","javascript:alertMessgae('用户未分配用户权限！')");
					}
				}
...
/*function getSubMenu(){
	var curM1=getURLParameter("m1");//当前页面地址一级标识
	$.get(contPath+"/portalSer/getSubMenus.do?m1="+curM1, function(data){
		var result = eval("("+data+")");
		var html="";
		if(result.flag=="ok"){
			var menuHtml=result.menuHtml;
...
function makeSubMenusHtml(menus,gsdjxh,dsdjxh){
	var html="<div class=\"title\">"+menus.cdmc+"<a href=\"#\" onclick=\"backZy()\" class=\"title-index\"><i class=\"iconfont fsicon-home\"></i>主页</a></div>";
				...
				for(var k=0;k<thirdMenus.length;k++){
					...
					if(thirdMenu.dkfs=="_blank"){//新窗口打开
						...
						if(mFlag3=="dwnsrswdjxxcj"||mFlag3=="gtjynsrswdjxxcj"||mFlag3=="clfxsxxcj"||mFlag3=="lsswdjnsrdj"||mFlag3=="kjywrdj"){
							...
							var params3="'"+makeRealUrl(secMenu.cdid,thirdMenu.cdid,thirdMenu,gsdjxh,dsdjxh)+"','"+menus.cdtb+"','"+thirdMenu.cdtb+"','','"+secMenu.qxkzsx+"'";
